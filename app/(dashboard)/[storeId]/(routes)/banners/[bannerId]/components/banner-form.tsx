...
export const BannerForm: React.FC<BannerFormProps> = ({ initialData }) => {
  ...
  const title = initialData ? "Edit Banner" : "Create New Banner";
  const description = initialData
    ? "Edit Store Banner"
    : "Create New Store Banner";
  const toastMessage = initialData
    ? "Banner successfully edit"
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.storeId}/banners/${params.bannerId}`); //delete data
      router.refresh();
      router.push("/");
